...
Name:        sidfile.py
Purpose:
    Usage 1: Provide a Class to handle SID and SuperSID formatted files
...
class SidFile():
    ...
    def __init__(self, filename="",
        ...
        2) A new empty file needs to be created: use 'sid_params'
            ...
            Usually this means you need to write file after data collection.
        ...
        if filename:
            ...
            try:
                with open(self.filename, "rt", encoding="ascii") as fin:
    ...
    def clear_buffer(self, next_day=False):
        """Create zero numpy arrays to receive data and generates timestamp."""
        if next_day:
            self.data.fill(0.0)
    ...
    def control_header(self):
        """Perform sanity check and assign standard attributes.

        Done in a format independent way.
        SuperSID files have an entry "Stations",
        while SID files have "StationID"
        """
        ...
        if "log_interval" in self.sid_params:
            ...
            print(
                "Warning: Log_Interval is missing! Please check. I assume 5 "
                "sec...")
    ...
    def set_all_date_attributes(self, keep_file_date=False):
        ...
        self.startTime = SidFile._StringToDatetime(
            self.sid_params["utc_starttime"])
    ...
    def read_header(self):
        ...
        for line in self.lines:
            ...
            tokens = line.split("=")
            if len(tokens) == 2:
                # remove the '#' and force the key to lower case
    ...
    def read_data(self, force_read_timestamp=False):
        """Using the self.lines buffer, converts data lines in numpy arrays.

            - One array self.data for the data (one column/vector per station)
            - One array self.timestamp for timestamps (i.e. timestamp vector)
        Reading method differs accordingly to the self.isSuperSID flag
        New: Extended format supports a timestamp for SuperSID format as well
        ...
        if self.isSuperSID and not self.is_extended:
            # classic SuperSID file format: one data column per station,
            # no time stamp (has to be generated)
            print(
                "Warning: read SuperSid non extended file and generate time "
                "stamps.")
            self.data = numpy.loadtxt(self.lines,
                                      comments='#',
                                      delimiter=",").transpose()
            self.generate_timestamp()
            ...
            # Try to avoid reading timestamps:
            ...
            if (((len(self.lines) - self.headerNbLines)
                ...
                print(
                    "Optimization: read SID file, generate timestamp instead "
                    "of reading & converting them from file.")
                self.data = numpy.array(numpy.loadtxt(self.lines, comments='#',
                                                      delimiter=",",
                                                      usecols=(1,)),
                                        ndmin=2)  # only read data column
                self.generate_timestamp()
    ...
    def get_sid_filename(self, station):
        ...
        For example RASPI_NWC_2013-08-31.csv
        """
        site = self.sid_params['site_name'] if 'site_name' in self.sid_params \
            else self.sid_params['site']
        return "%s_%s_%s.csv" % (site, station,
                                 self.sid_params["utc_starttime"][:10])

    def get_supersid_filename(self):
        """Return a file name as <Site Name>__<UTC Start Date>.csv.

        For example RASPI_2013-08-31.csv
        """
        site = self.sid_params['site_name'] if 'site_name' in self.sid_params \
            else self.sid_params['site']
    ...
    def get_station_index(self, station):
        ...
        if type(station) is int:
            ...
            return self.stations.index(station['call_sign'])
        else:
            # throw a ValueError if 'station' is not in the list
            return self.stations.index(station.call_sign)

    def copy_data(self, second_sidfile):
        """Copy the second_sidfile's data on the current data vector.

            Do this for every common stations.
        ...
        for iStation, station in enumerate(self.stations):
            try:
                ...
                # deep copy
                self.data[iStation] = second_sidfile.data[second_idx][:]
                has_copied = True
            except ValueError:
    ...
    def create_header(self, isSuperSid, log_type):
        ...
        """
        hdr = "# Site = %s\n" % (self.sid_params['site_name']
                                 if 'site_name' in self.sid_params
                                 else self.sid_params['site'])
        ...
        hdr += "# TimeZone = %s\n" % (self.sid_params['time_zone']
                                      if 'time_zone' in self.sid_params
                                      else self.sid_params['timezone'])
        hdr += "#\n"
        hdr += "# UTC_StartTime = %s\n" % self.sid_params['utc_starttime'][:19]
        hdr += "# LogInterval = %s\n" % (self.sid_params['log_interval']
                                         if 'log_interval' in self.sid_params
                                         else self.sid_params['loginterval'])
        hdr += "# LogType = %s\n" % log_type
        hdr += "# MonitorID = %s\n" % (self.sid_params['monitor_id']
                                       if 'monitor_id' in self.sid_params
                                       else self.sid_params['monitorid'])
        if isSuperSid:
            # if only one station is configured,
            # self.sid_params['stations'] is not present
            if 'stations' in self.sid_params:
    ...
    def write_data_sid(self, station, filename, log_type, apply_bema=True,
                       extended=False, bema_wing=6):
        """Write in the file 'filename' the dataset of the given station.

        Write using the SID format
            i.e. "TimeStamp, Data" lines
        Header respects the SID format definition
            i.e. conversion if self is SuperSid
        """
        iStation = self.get_station_index(station)
        #  if the exiting file is SuperSID
        # need extra information to create the header's SID file parameters
        if self.isSuperSID:
        ...
        with open(filename, "wt") as fout:
            ...
            for t_stamp, x in zip(self.timestamp, tmp_data):
                print("%s, %.15f" % (t_stamp.strftime(timestamp_format), x),
                      file=fout)

    def write_data_supersid(self, filename, log_type, apply_bema=True,
                            extended=False, bema_wing=6):
        """Write the SuperSID file.

        Attention: self.sid_params must contain all expected entries.
        """
        ...
        with open(filename, "wt") as fout:
            ...
            if log_type == RAW or not apply_bema:
                ...
                for stationData in self.data:
                    tmp_data.append(SidFile.filter_buffer(stationData,
                                                          self.LogInterval,
                                                          bema_wing=bema_wing))
                tmp_data = numpy.array(tmp_data)
            # print(tmp_data.shape) should be like (2, 17280)
            if extended:
                for t_stamp, row in zip(self.timestamp,
                                        numpy.transpose(tmp_data)):
                    floats_as_strings = ["%.15f" % x for x in row]
                    print(t_stamp.strftime(SidFile._TIMESTAMP_EXTENDED+","),
                          ", ".join(floats_as_strings), file=fout)
            else:
        ...
        # future: version 1.7 offers "header=hdr" as new function param
        # but for now (1.6) we write header first then savetxt() append data
        # numpy.savetxt(filename, tmp_data, delimiter=",",
        #               newline="\n", header=hdr)

    @classmethod
    def filter_buffer(
            cls, raw_buffer, data_interval, bema_wing=6, gmt_offset=0):
        """
        Return bema filtered version of the buffer,
        with optional time_zone_offset.
        BEMA filter uses the minimal found value to represent the
        data points within a range (bema_window)
        bema_wing = 6 => window = 13
        (bema_wing + evaluating point + bema_wing)
        """
        length = len(raw_buffer)
        # Extend 2 wings to  raw data buffer before taking min and average
        ...
        # dstack[bema_wing]
        dstack[0:bema_wing] = raw_buffer[0]

        # dstack[length+bema_wing-1]
        dstack[length+bema_wing:length+bema_wing*2] = raw_buffer[-1]
        ...
        if gmt_offset == 0:
            ...
            doffset = numpy.hstack((daverage[gmt_mark:length],
                                    daverage[0:gmt_mark]))
            return doffset
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        required=False,
        type=exist_file,
        help="Split 1 supersid_format in N sid_format / 1 supersid_extended "
        "in N sid_extended i.e. one per station")
    parser.add_argument(
        "-m", "--merge",
        dest="filename_merge",
        required=False,
        type=exist_file,
        nargs="*",
        help="Merge 2 sid_format in one supersid_format / 2 supersid_extended "
        "in one supersid_extended")
    parser.add_argument(
        "-i", "--info",
        dest="filename_info",
        required=False,
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Width of the window used in filtering a.k.a. 'bema_wing' "
        "(default=6)")
    ...
    if args.filename_info:
        ...
        for i in range(len(sid.timestamp)):
            if sid.data[0][i] != 0.0:
                break
        if i == len(sid.timestamp):
            i = 0
        for t_stamp, row in zip(sid.timestamp[i:i+5],
                                numpy.transpose(sid.data)[i:i+5]):
            floats_as_strings = ["%.15f" % x for x in row]
            print(t_stamp.strftime(sid.timestamp_format+","),
                  ", ".join(floats_as_strings))
    ...
    # Some 'real' manipulations:
        ...
        for station in sid.stations:
            ...
            sid.write_data_sid(station, fname,
                               sid.sid_params['logtype'],
                               apply_bema=False)
            print(fname, "created.")
        ...
        # merge the SuperSID's matching station to SID file
            ...
            if station not in supersid.stations:
                print(
                    "Error: station %s in not found in the superSId file. "
                    "Cannot merge." % station)
            else:
                sid.data += supersid.get_station_data(station)
                sid.write_data_sid(station, fmerge(sid.filename),
                                   sid.sid_params['logtype'], apply_bema=False)
                print(fmerge(sid.filename), "created.")
        # 2 SID files to merge in one SID file - sid1's header is kept
        else:
            sid1.data += sid2.data
            sid1.write_data_sid(sid1.stations[0], fmerge(sid1.filename),
                                sid1.sid_params['logtype'], apply_bema=False)
            print(fmerge(sid1.filename), "created.")
    elif args.filename_filter:
        # Convert a RAW file into a Filtered one.
        # Can filter an already filtered file too...
        ...
        if sid.sid_params['logtype'] != RAW:
            print(
                "Warning: %s is not a raw file. This might filter an already "
                "filtered file." % args.filename_filter)
        bema_wing = args.bema_wing if args.bema_wing else 6
        if sid.isSuperSID:
            sid.write_data_supersid(fname, log_type=FILTERED, apply_bema=True,
                                    extended=sid.is_extended,
                                    bema_wing=bema_wing)
        else:
            sid.write_data_sid(sid.stations[0], fname, log_type=FILTERED,
                               apply_bema=True, extended=sid.is_extended,
                               bema_wing=bema_wing)
    else:
        parser.print_help()
