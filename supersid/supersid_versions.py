...
if __name__ == '__main__':
    ...
    for module in tested_versions:
        try:
            if module == 'python':
                identified_versions[module] = "{}.{}.{}".format(sys.version_info.major, sys.version_info.minor, sys.version_info.micro)
            elif (module in sys.builtin_module_names):
                identified_versions[module] = 'builtin'
            else:
                identified_versions[module] = pkg_resources.get_distribution(module).version

            if identified_versions[module] in tested_versions[module]['versions']:
                print("SUCCESS: {} '{}' found version {}".format(tested_versions[module]['requirement'], module, identified_versions[module]))
            else:
                print("WARNING: {} '{}' found version {}, expected version(s) {}".format(tested_versions[module]['requirement'], module, identified_versions[module], tested_versions[module]['versions']))
        except Exception as e:
            if 'mandatory' == tested_versions[module]['requirement']:
                print("ERROR: {} '{}' not found".format(tested_versions[module]['requirement'], module))
            else:
                print("INFO: {} '{}' not found".format(tested_versions[module]['requirement'], module))
